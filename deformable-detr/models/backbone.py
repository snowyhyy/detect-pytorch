...
LastEditTime: 2025-07-18 09:49:47
Description: 根据官方的实现，添加中文注释，源码：https://github.com/facebookresearch/detr.git，https://github.com/fundamentalvision/Deformable-DETR.git
'''
...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, train_backbone: bool, return_interm_layers: bool):
        ...
        for name, parameter in backbone.named_parameters():
            if not train_backbone or 'layer2' not in name and 'layer3' not in name and 'layer4' not in name: # 训练 backbone 网络时，解冻 layer2 layer3 layer4 的参数
...
class Backbone(BackboneBase):
    ...
    def __init__(self, name: str,
                 ...
                 dilation: bool):
        backbone = getattr(torchvision.models, name)( # 动态加载模型  # 在默认目录下如果没有对应网络权重，会自动下载
        ...
        super().__init__(backbone, train_backbone, return_interm_layers) # 继承 BackboneBase 类
        if dilation: # 如果对 layer4 使用空洞卷积，则输出尺寸为输入的 1 / 16
...
def build_backbone(args):
    position_embedding = build_position_encoding(args) # 构建位置编码层
    train_backbone = args.lr_backbone > 0 # 如果 backbone 网络的学习率大于 0，则要训练 backbone 网络的参数。
    return_interm_layers = args.masks or (args.num_feature_levels > 1) # 是否返回中间层特征：args.masks 是用于分割的，分割也要用中间层特征
